    ...
    public struct PointyHexagonGrid : IMeshGenerator
    {
        ...
        public void Execute<S>(int z, S streams) where S : struct, IMeshStreams
        {
            ...
            for (int x = 0; x < Resolution; x++, vi +=7, ti +=6)
            {
                ...
                vertex.uv0 = float2(0.5f - h, 0.25f);
                ...
                vertex.uv0 = float2(0.5f ,1f);
                streams.SetVertex(vi + 4, vertex);

                vertex.position.x = xCoordinates.y;
                vertex.position.z = zCoordinates.z;

                vertex.uv0 = float2(0.5f + h, 0.75f);
        ...
        public Bounds Bounds => new Bounds(
            Vector3.zero, new Vector3(
                (Resolution > 1 ? 0.5f + 0.25f / Resolution : 0.5f) * sqrt(3f),0f,0.75f + 0.25f /Resolution )
            );
    };
}
